...
class Body extends StatefulWidget {
  ...
  const Body({Key key, @required this.totalRewards}) : super(key: key);
  @override
  ...
  Widget _rewardsGrid() {
    return StreamBuilder(
        stream: Firestore.instance
            .collection('users/${userData.uid}/rewards')
            .snapshots(),
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          ...
          if (snapshot.hasData) {
            if (snapshot.data.documents.length <= 0) {
              return CustomCenter(
                child: Text("You didn't get any reward."),
                extraValue: 160,
              );
            }
          }
